...
void Hud::update_health_bar(const Player& player) {
    content.health_bar.set_size(Vector2f(player.get_health() * 3.f, 50.f));
}
...
void Hud::update_ammo(const Weapon& weapon) {
    ...
    content.text_ammo.set_string(stream_ammo.str());
}
...
void Hud::update_scores() {
    ...
    content.text_score.set_string(stream_score.str());

    stream_high_score << "High score: " << Score::get_instance().get_high_score();
    content.text_high_score.set_string(stream_high_score.str());
}
...
void Hud::update_horde(const Horde& horde) {
    ...
    content.text_wave_number.set_string(stream_wave.str());

    stream_zombies_alive << "Zombies: " << horde.num_zombies_alive;
    content.text_zombies_remaining.set_string(stream_zombies_alive.str());
}
} // namespace game
