...
struct HudContent {
    ...
    TextWrapper text_zombies_remaining{"Zombies: ", font, 55, Color::White,
                                       Vector2f(1500.f, 980.f)};
...
class Hud
{
  public:
    Hud() = default;
    void update(const Game& game, const Player& player, const Weapon& weapon, const Horde& horde);
    HudContent get_content() const { return content_; }

  private:
    void update_scores(const Game& game);
