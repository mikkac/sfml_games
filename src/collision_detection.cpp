...
namespace game
{
bool CollisionDetection::detect(LevelManager* level_manager, PlayableCharacter* character) {
...
void CollisionDetection::create_collision_zone() {
    ...
    start_zone_ = Vector2i{static_cast<int>(zone.left / kTileSize) - 1,
                           static_cast<int>(zone.top / kTileSize) - 1};
    end_zone_ = Vector2i{static_cast<int>(zone.left / kTileSize) + 2,
                         static_cast<int>(zone.top / kTileSize) + 3};
}
...
void CollisionDetection::handle_leaving_level() {
    ...
    if (end_zone_.x >= static_cast<int>(level_manager_->get_level_size().x))
        end_zone_.x = level_manager_->get_level_size().x;
    if (end_zone_.y >= static_cast<int>(level_manager_->get_level_size().y))
        end_zone_.y = level_manager_->get_level_size().y;
    ...
    if (not character_->get_position().intersects(level))
...
bool CollisionDetection::endgame_block() {
    ...
    if (current_block_type_ == BlockType::ENDGAME) return true;
}
} // namespace game
